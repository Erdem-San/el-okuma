      ...
      <div className="mystic-card p-6 w-full max-w-xs space-y-4 animate-fade-in-up" style={{ border: '1px solid var(--border-gold)' }}>
        <div className="text-center space-y-2">
          ...
          <p className="text-sm" style={{ color: 'var(--cream-dim)' }}>
            <span style={{ color: 'var(--gold)' }}>{name}</span> için yapılan el falı okuma ve tüm konuşma geçmişi kalıcı olarak silinecek.
          </p>
        </div>
  ...
  return (
    <div className="mystic-card p-4 flex items-center gap-4 active:scale-[0.99] transition-transform">
      ...
      <div className="w-12 h-12 rounded-xl flex items-center justify-center text-xl flex-shrink-0"
        style={{ background: 'linear-gradient(135deg, rgba(201,169,110,0.12), rgba(124,77,138,0.08))', border: '1px solid var(--border-gold)' }}>
        {hasResult ? '🔮' : '🖐'}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs mt-0.5" style={{ color: hasResult ? 'var(--gold)' : 'var(--cream-dim)', opacity: hasResult ? 0.8 : 0.5 }}>
          {STEP_LABELS[session.step] ?? session.step}
        </p>
        <p className="text-xs mt-0.5" style={{ color: 'var(--cream-dim)', opacity: 0.35 }}>
          {date} · {photoCount} fotoğraf{chatCount > 0 ? ` · ${chatCount} mesaj` : ''}
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        <button
          ...
          style={{ background: 'linear-gradient(135deg, rgba(138,92,42,0.3), rgba(201,169,110,0.2))', border: '1px solid var(--border-gold)', color: 'var(--gold)' }}
        >
        ...
        <div className="my-auto w-full space-y-8">
          ...
          <div className="text-center space-y-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm" style={{ color: 'var(--cream-dim)' }}>
                Yapay zeka ile ellerinizin sırlarını keşfedin.
              </p>
            </div>

            <button onClick={handleNewSession} className="btn-mystic-primary w-full py-4 text-base">
              🔮 Yeni El Falı Başlat
            </button>
          </div>
